...
/**
 * Controller for managing airline data.
 * It provides endpoints to retrieve, add, delete, and update airline information.
...
public class AirlineController {
   ...
   *
   * @return Return a ResponseEntity containing a list of airlines
   *      or an empty list if no airlines exist.
   */
  @GetMapping
  ...
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = "204",
          description = "No Airlines are available, so no content.",
          content = @Content
      ),
   ...
   * @return Return a ResponseEntity containing the requested airline
   *      or not found status if it does not exist.
   */
  @GetMapping("/{id}")
  @Operation(
      summary = "Get Airline by id",
      description = "Uses the unique id to find the desired Airline"
  )
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "The Airline return in the response body."),
      @ApiResponse(
          responseCode = "404",
          description = "No Airline are available, not found.",
          content = @Content
      )
  })
  ...
  @Operation(
      summary = "Add an Airline",
      description = "Send Json data about the airline to be saved to the database",
  ...
  @Operation(
      ...
      description = "Delete an airline based on the unique id provided",
      security = @SecurityRequirement(name = "bearerAuth")
  )
  @ApiResponses(value = {
  ...
  @Operation(
      ...
      description = "Update an airline based on the unique id provided",
      security = @SecurityRequirement(name = "bearerAuth")
  )
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "The Airline return in the response body."),
      @ApiResponse(
          responseCode = "400",
          description = "The Airline is invalid.",
          content = @Content
      ),
      @ApiResponse(
          responseCode = "404",
  ...
  public ResponseEntity<Airline> updateAirline(
      ...
      @RequestBody Airline updatedAirline
  ) {
    ResponseEntity<Airline> response;
